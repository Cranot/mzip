...
void Predictor::update(int y) {
  ...
  static U32 wordh=0,cword=0,pword=0,grp=0,caseh=0,line0=0,line1=0,container=0,tagacc=0,prevcont=0;  // word+case+line+wiki
  ...
  if (c0>=256) {
    ...
    { int low=(c0>=97&&c0<=122); if(c0>=65&&c0<=90){c0+=32;low=1;}   // word stack
      ...
      else { if(wordh){pword=cword;cword=wordh;} wordh=0; } }
    ...
    // wiki contexts. -DWIKIONE=k emits ONLY context k (isolation); else cumulative by NWIKI.
#ifdef WIKIONE
  #if WIKIONE==0
    h[19+XPN+0]=((U32)wmode2*2654435761u)^(wordh*40503u)^(U32)(c4&0xff);     // richmode x word
  #elif WIKIONE==1
    h[19+XPN+0]=(container*0x9E3779B1u)^(wordh*40503u);                      // container x word
  #elif WIKIONE==2
    h[19+XPN+0]=(container*2654435761u)^(U32)(c4&0xff);                      // container x lastbyte
  #elif WIKIONE==3
    h[19+XPN+0]=((U32)listc*2246822519u)^(U32)(c4&0xff);                     // list marker
  #elif WIKIONE==4
    h[19+XPN+0]=(container*40503u)^(grp*2654435761u);                        // container x class
  #elif WIKIONE==5
    h[19+XPN+0]=((U32)wmode2*0x9E3779B1u)^(grp*2246822519u);                 // richmode x class
  #elif WIKIONE==6
    h[19+XPN+0]=(container*2654435761u)^((U32)wmode2*40503u);                // container x richmode
  #endif
#else
  #if NWIKI>=1
    h[19+XPN+0]=((U32)wmode2*2654435761u)^(wordh*40503u)^(U32)(c4&0xff);     // richmode x word
  #endif
  #if NWIKI>=2
    h[19+XPN+1]=(container*0x9E3779B1u)^(wordh*40503u);                      // container x word
  #endif
  #if NWIKI>=3
    h[19+XPN+2]=(container*2654435761u)^(U32)(c4&0xff);                      // container x lastbyte
  #endif
  #if NWIKI>=4
    h[19+XPN+3]=((U32)listc*2246822519u)^(U32)(c4&0xff);                     // list marker
  #endif
  #if NWIKI>=5
    h[19+XPN+4]=(container*40503u)^(grp*2654435761u);                        // container x class
  #endif
  #if NWIKI>=6
    h[19+XPN+5]=((U32)wmode2*0x9E3779B1u)^(grp*2246822519u);                 // richmode x class
  #endif
  #if NWIKI>=7
    h[19+XPN+6]=(container*2654435761u)^((U32)wmode2*40503u);                // container x richmode
  #endif
#endif
#endif
#ifdef IDNUM
  ...
  for(int i=0;i<NM;i++) m.add(stretch(sm[i].p(y, *cp[i])));
