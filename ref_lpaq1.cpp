...
void Predictor::update(int y) {
  ...
  static HashTable<16> t(1ull<<32);  // cxt -> state (4GB)
  static int c0=1;  // last 0-7 bits with leading 1
  static int c4=0;  // last 4 bytes
  static unsigned long long c8=0;  // last 8 bytes
  static U32 wordh=0,cword=0,pword=0,grp=0,caseh=0,line0=0,line1=0;  // word+case+line state
  static int bcount=0,wpos=0,col=0,ic=0;
  static U8 *cp[19]={t0,t0,t0,t0,t0,t0,t0,t0,t0,t0,t0,t0,t0,t0,t0,t0,t0,t0,t0};
  static StateMap sm[19];
  static APM a1(0x100), a2(0x4000), a3(0x1000);
  static U32 h[19];
  static Mixer m(20, 512);
  static MatchModel mm(1u<<30, 1u<<28);  // 1GB match buffer covers enwik9 + 1GB index
  assert(MEM>0);

  // update model
  assert(y==0 || y==1);
  for(int i=0;i<19;i++) *cp[i]=nex(*cp[i], y);
  m.update(y);
  ...
  if (c0>=256) {
    ...
    for(int i=1;i<19;i++) cp[i]=t[h[i]]+1;
    c0=1;
    bcount=0;
  }
  if (bcount==4) {
    for(int i=1;i<19;i++) cp[i]=t[h[i]+c0]+1;
  }
  else if (bcount>0) {
    int j=y+1<<(bcount&3)-1;
    for(int i=1;i<19;i++) cp[i]+=j;
  }
  cp[0]=t0+h[0]+c0;
  ...
  for(int i=0;i<19;i++) m.add(stretch(sm[i].p(y, *cp[i])));
