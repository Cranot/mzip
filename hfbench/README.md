...
- **Net, fully scripted (R130): 1,490 TB held today → 1,254 TB** for the sampled week, 15.8% less (5–95%: 11–23%), 236 TB/week, 12.3 PB/year. 1,211 TB (18.7%) adding undeclared twins; 1,208 TB (18.9%) adding every readable "solo" BF16 upload resolved to a 546-root index (R128). **22.2% / 320 TB is withdrawn**: finalnum2..10 carried two literals (`HELD_NOW = 1444`, `OTHER_SAVE = 165.7`) that no table on disk reproduces; the derived baseline is 1,490 TB and the non-GGUF per-chunk floors sum to 111 TB. The Xet side was checked with xet-core's own chunker and encoders (`xet/xetratio.rs`): BF16 within 0.3%, GGUF within 0.1% of the modelled ratios.
- **The unreadable pool is one base.** 129 gated uploads at Qwen3.6-35B-A3B's exact byte size (or minus its MTP head) are 13% of the week's weight bytes (R126). Priced through six readable siblings the week reaches 1,153 TB (22.6%, 5–95% 16–32%) — an inference, n=6, not a measurement.
...
| path | what |
|---|---|
| `MZIP-AT-HF-GRANULARITY.txt` | the record, 162 numbered RESULTs |
...
| `bootstrap2.py`, `otherterm.py`, `xet/xetratio.rs` | the derived baseline, the 5–95% intervals on every row, and Xet's own chunker+encoders run on our files (R130) |
