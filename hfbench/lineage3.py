...
def slice_of(rid):
    """64 KiB of the largest 2D non-embedding BF16/F16 tensor, from the first shard that has one"""
    ...
    for fn in shards[:3]:
        ...
        for k, v in hb.items():
            ...
            if sz < 65536: continue
            if best is None or sz > best[1]: best = (k, sz, v)
...
def main():
    ...
    for key, mem in groups.items():
        ...
        for (ra, ba, da, wa), (rb, bb, dbb, wb) in itertools.combinations(mem, 2):
            ...
            c = float(np.corrcoef(wa[:m], wb[:m])[0, 1]) if np.std(wa[:m]) > 0 and np.std(wb[:m]) > 0 else 0.0
    ...
    json.dump(dict(probed=n, probed_b=probed_b, tot_w=tot_w, exact_b=exact_b, pert_b=pert_b,
              ...
              open(f"{B}/qgq/lineage3.json", "w"), indent=1)
    print("LINEAGE3_DONE")
